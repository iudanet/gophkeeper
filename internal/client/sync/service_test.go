...
// mockAPIClient - simple hand-written mock for APIClient
type mockAPIClient struct {
	syncFunc    func(ctx context.Context, accessToken string, req api.SyncRequest) (*api.SyncResponse, error)
	lastRequest *api.SyncRequest
	syncCalled  bool
}

func (m *mockAPIClient) Sync(ctx context.Context, accessToken string, req api.SyncRequest) (*api.SyncResponse, error) {
	m.syncCalled = true
	m.lastRequest = &req
	if m.syncFunc != nil {
		return m.syncFunc(ctx, accessToken, req)
	}
...
// mockMetadataStorage - simple hand-written mock for MetadataStorage
type mockMetadataStorage struct {
	saveLastSyncTimestampErr error
	getLastSyncTimestampErr  error
...
func (m *mockMetadataStorage) SaveLastSyncTimestamp(ctx context.Context, timestamp int64) error {
	if m.saveLastSyncTimestampErr != nil {
		return m.saveLastSyncTimestampErr
	}
	m.lastSyncTimestamp = timestamp
	return nil
}

func (m *mockMetadataStorage) GetLastSyncTimestamp(ctx context.Context) (int64, error) {
	if m.getLastSyncTimestampErr != nil {
		return 0, m.getLastSyncTimestampErr
	}
	return m.lastSyncTimestamp, nil
}

// mockCRDTStorage - simple hand-written mock for CRDTStorage
type mockCRDTStorage struct {
	entries                     map[string]*models.CRDTEntry
	getEntriesAfterTimestampErr error
	saveEntryErr                error
	getEntryErr                 error
}
...
func (m *mockCRDTStorage) GetEntriesAfterTimestamp(ctx context.Context, timestamp int64) ([]*models.CRDTEntry, error) {
	if m.getEntriesAfterTimestampErr != nil {
		return nil, m.getEntriesAfterTimestampErr
	}
