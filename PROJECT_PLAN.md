...
- [ ] **Инициализация SQLite с правильными настройками:**
  - [ ] Создать `internal/server/storage/db.go`:
    ```go
    func OpenDB(path string) (*sql.DB, error) {
        // DSN с pragma для WAL и других настроек
        ...
        // КРИТИЧНО: MaxOpenConns = 1 для SQLite
        db.SetMaxOpenConns(1)

        return db, db.Ping()
    }
    ```
  - [ ] **ВАЖНО: MaxOpenConns = 1** - предотвращает "database is locked"
  - [ ] **WAL mode** - для производительности и параллельного чтения
  - [ ] **busy_timeout = 5000ms** - вместо немедленной ошибки при блокировке
